...
class MotionDataset(Dataset):
    def __init__(self, motion_path = './motion_data/run1', side_length=256, sequence_range=[5,6], skip = 4):
        ...
        self.coords = torch.stack(
            torch.meshgrid(
                [
                    ...
                    torch.linspace(-1.0, 1.0, side_length),
                    torch.linspace(-0.01, 0.01, frames),
                ]
            ),
            dim=-1,
...
class HDRDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Any = None):
        self.dataset = MotionDataset(side_length=self.side_length)
        return
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='motion_xyt',save_last=True, save_top_k=1, monitor="train_psnr", mode="max", every_n_epochs=100)
    model = PLFourierNet()
    ckpt_path = "/home/xxy/Documents/code/Fourier-Manifold/motion_xyt/last-v4.ckpt"
