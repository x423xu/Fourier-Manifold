...
class MFNBase(nn.Module):
    ...
    def forward(self, x, z):
        out = self.filters[0](x+self.phase_modulator[0](z), self.frequency_modulator[0](z))
        z_0 = self.amplitude_modulator[0](z)
        out_z = out*z_0
        out_y = out
        for i in range(1, len(self.filters)):
            out_z = self.amplitude_modulator[i](z)+self.filters[i](x+self.phase_modulator[i](z),self.frequency_modulator[i](z)) * self.linear[i - 1](out_z)
        for i in range(1, len(self.filters)):
...
class PLFourierNet(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        for y in y_pred:
            ...
            y = y.astype(np.uint8)
            ys.append(y)
        imageio.mimsave(f'test_phase.gif', ys, duration = 300)
...
if __name__ == "__main__":
    ...
    model = PLFourierNet()
    # ckpt_path = "/home/xxy/Documents/code/Fourier-Manifold/motion_phase_pos_embed/checkpoints/20230807_15:26:34.ckpt"
    ...
    trainer = pl.Trainer(accelerator='gpu' ,max_epochs=3000, devices = 1, log_every_n_steps=100, enable_progress_bar=True, callbacks=[checkpoint_callback],logger=logger)
