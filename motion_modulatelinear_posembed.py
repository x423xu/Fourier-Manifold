...
class MFNBase(nn.Module):
    ...
    def forward(self, x, z):
        ...
        for i in range(1, len(self.filters)):
            out_z = self.filters[i](x) * (self.linear[i - 1](out_z)+self.linear_modulator[i-1](z))
        for i in range(1, len(self.filters)):
...
class FourierNet(MFNBase):
    def __init__(
        ...
        self.linear_modulator = nn.ModuleList(
            [
...
class PLFourierNet(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        for y in y_pred:
            ...
            y = y.astype(np.uint8)
            ys.append(y)
        imageio.mimsave(f'test.gif', ys, duration = 100)
...
if __name__ == "__main__":
    kwargs = {
        'project_name': 'motion_linear_pos_embed',
        'batch_size': 20,
        'side_length': 256,
        'pos_embed': False,
    ...
    model = PLFourierNet()
    ckpt_path = "/home/xxy/Documents/code/Fourier-Manifold/motion_linear_pos_embed/checkpoints/last.ckpt"
